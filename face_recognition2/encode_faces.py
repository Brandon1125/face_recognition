...
def codifica_caras(detection="hog"):
    # grab the paths to the input images in our dataset
    print('[INFO] quantifying faces...')
    imagePaths = list(paths.list_images("../face_recognition/dataset"))
   #imagePaths = list(paths.list_images("dataset"))
    ...
    with open("../face_recognition/encoded_faces.pickle", 'wb') as file:
        file.write(pickle.dumps(data))
    #with open("encoded_faces.pickle", 'wb') as file:
        #file.write(pickle.dumps(data))
        
    print('[INFO] encodings saved!, number of faces: ', len(imagePaths))
