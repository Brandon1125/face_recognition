...
def recognition_liveness(model_path, le_path, detector_folder, encodings, confidence=0.9):
    ...
    vs = cv2.VideoCapture("http://192.168.43.92:81/stream")
    ...
    while True:
        ...
        for i in range(0, detections.shape[2]):
            ...
            if confidence > args['confidence']:
                ...
                if name == 'Unknown' or label_name == 'fake':
                    sequence_count = 0
                elif name != 'Unknown' and label_name == 'real' and preds[j] >= 0.705:
                    sequence_count += 1
                print(f'[INFO] {name}, {label_name}, seq: {sequence_count}, confidence {preds[j]:.4f}')
                
                if label_name == 'fake':
                    cv2.putText(frame, "No trates de engañar!", (startX, endY + 25), 
        ...
        if cv2.waitKey(1) & 0xFF == ord('q') or sequence_count==15:
            break
...
if __name__ == '__main__':
    name, label_name = recognition_liveness('liveness.model', 'label_encoder.pickle', 
                                            'face_detector', '../face_recognition/encoded_faces.pickle',
                                            confidence=0.5)
